#include "hzpch.hpp"
#include "Platform/GLFW/WindowsWindow.hpp"
...
{
	Hazel::Console::CoreError("GLFW Error ({0}): {1}", error, description);
}

Hazel::Scope<Hazel::Window> Hazel::Window::Create(const WindowProps& props)
{
...
void Hazel::WindowsWindow::Init(const WindowProps& props)
{
	...
	Console::CoreLog("Creating window {0} ({1}, {2})", props.Title, props.Width, props.Height);
